...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet packetIn) throws Exception {
        if (PacketUtils.INSTANCE.getPacketType(packetIn) == PacketUtils.PacketType.SERVERSIDE) {
            new PacketEvent(packetIn, PacketEvent.Type.RECEIVE).callEvent(); // call packetEvent (receive packet)
        }
    ...
    public void sendPacket(Packet packetIn) {
        if (PacketUtils.INSTANCE.getPacketType(packetIn) == PacketUtils.PacketType.CLIENTSIDE) {
            new PacketEvent(packetIn, PacketEvent.Type.SEND).callEvent(); // call packetEvent (send packet)
        }
    ...
    public static NetworkManager createNetworkManagerAndConnect(InetAddress address, int serverPort, boolean useNativeTransport) {
        ...
        if (ProxyManager.getInstance().state) {
            ...
            bootstrap.group(eventLoopGroup).handler(new ChannelInitializer<>() {
